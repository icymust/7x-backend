...
import { ABU_DHABI_CENTER, ABU_DHABI_ZOOM } from '../data/mapData'
...
const warehouseBounds = warehouses.features.reduce(
...
onMounted(() => {
  ...
  map.value = m
  m.on('load', () => {
    m.fitBounds(warehouseBounds, { padding: 28, duration: 0 })
    addMarkers()
  })
})
